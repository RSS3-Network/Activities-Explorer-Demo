...
import { getActivities, type Activity } from "@rss3/sdk";
	...
	return (
		<div className="max-w-2xl mx-auto px-4 py-6">
			<div className="flex flex-col items-center justify-center gap-4 w-full mb-6">
				<img
					className="w-[120px]"
					src="https://cdn.jsdelivr.net/gh/rss3-network/rss3-assets/logo.svg"
					alt="RSS3 logo"
				/>
				<h1 className="text-3xl font-bold text-center">
					Explore <span className="text-[#0072ff]">Web3</span> activities
				</h1>
								...
								>
										...
										<div className="px-4 py-2 bg-gray-50 text-xs text-gray-700">
											...
											<div className="mt-2 flex items-center space-x-2">
												{activity.actions[0].from && (
													<div className="group relative">
														<span className="cursor-pointer">
															From: {activity.actions[0].from.slice(0, 6)}...
															{activity.actions[0].from.slice(-4)}
														</span>
												...
												{activity.actions[0].to && (
													<div className="group relative">
														<span className="cursor-pointer">
															To: {activity.actions[0].to.slice(0, 6)}...
															{activity.actions[0].to.slice(-4)}
														</span>
														<span className="absolute left-0 top-full mt-1 w-48 rounded bg-gray-800 px-2 py-1 text-white text-xs opacity-0 transition-opacity group-hover:opacity-100">
															{activity.actions[0].to}
														</span>
									...
									{activity.actions[0].relatedUrls[0] && (
										<div className="px-4 py-2 bg-gray-100">
											<a
												href={activity.actions[0].relatedUrls[0]}
												target="_blank"
												rel="noopener noreferrer"
												className="text-xs text-blue-600 hover:underline"
											>
												View on{" "}
												{new URL(activity.actions[0].relatedUrls[0]).hostname}
											</a>
										</div>
									)}
								</li>
